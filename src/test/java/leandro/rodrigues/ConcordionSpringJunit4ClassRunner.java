...
public class ConcordionSpringJunit4ClassRunner extends SpringJUnit4ClassRunner {
	...
	public ConcordionSpringJunit4ClassRunner(Class<?> fixtureClass)
			throws InitializationError, NoSuchMethodException {
		super(fixtureClass);
		String testDescription = ("[Concordion Specification for '" + fixtureClass
				.getSimpleName()).replaceAll("Test$", "']");
		fixtureDescription = Description.createTestDescription(fixtureClass,
				testDescription);
		fakeMethod = new FrameworkMethod(this.getClass().getDeclaredMethod(
				"computeTestMethods"));
	}
